...
if defined ZSL_PYTHON (
  "%ZSL_PYTHON%" process_events_windows.py --input-root "%DATA_ROOT%" --scan-only %OVERRIDE_ARGS%
) else (
  call "%CONDA_CMD%" run --no-capture-output -n zsl-windows python process_events_windows.py --input-root "%DATA_ROOT%" --scan-only %OVERRIDE_ARGS%
)
...
:usage
echo 用法：%~nx0 "数据总目录" ["event_overrides.json"]
exit /b 2
